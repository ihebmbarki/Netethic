...
extension UIViewController {
    ...
    func revealViewController() -> Events? {
        var viewController: UIViewController? = self

        if viewController != nil && viewController is Events {
            return viewController! as? Events
        }
        while (!(viewController is Events) && viewController?.parent != nil) {
            viewController = viewController?.parent
        }
        if viewController is Events {
            return viewController as? Events
        }
        return nil
    }
...
extension Events: UIGestureRecognizerDelegate {
