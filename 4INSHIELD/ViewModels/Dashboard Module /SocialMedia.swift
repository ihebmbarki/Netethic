...
extension UIViewController {
    ...
    func revealViewController() -> SocialMedia? {
        var viewController: UIViewController? = self

        if viewController != nil && viewController is SocialMedia {
            return viewController! as? SocialMedia
        }
        while (!(viewController is SocialMedia) && viewController?.parent != nil) {
            viewController = viewController?.parent
        }
        if viewController is SocialMedia {
            return viewController as? SocialMedia
        }
        return nil
    }
...
extension SocialMedia: UIGestureRecognizerDelegate {
